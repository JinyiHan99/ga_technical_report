\definecolor{caseprompttitle}{RGB}{196,129,44}
\definecolor{casepromptbg}{RGB}{248,239,220}
\definecolor{casetracetitle}{RGB}{67,119,183}
\definecolor{casetracebg}{RGB}{231,239,250}
\definecolor{caseanswertitle}{RGB}{76,133,68}
\definecolor{caseanswerbg}{RGB}{231,242,229}

\newtcolorbox{CasePromptBox}[1]{
  breakable,
  colback=casepromptbg,
  colframe=caseprompttitle,
  colbacktitle=caseprompttitle,
  coltitle=white,
  fonttitle=\bfseries,
  title={#1}
}
...
\newtcolorbox{CaseTraceBox}[1]{
  breakable,
  colback=casetracebg,
  colframe=casetracetitle,
  colbacktitle=casetracetitle,
  coltitle=white,
  fonttitle=\bfseries,
  title={#1}
}
...
\newtcolorbox{CaseAnswerBox}[1]{
  breakable,
  colback=caseanswerbg,
  colframe=caseanswertitle,
  colbacktitle=caseanswertitle,
  coltitle=white,
  fonttitle=\bfseries,
  title={#1}
}
...
Here we provide expanded case-study exhibits for the qualitative behaviors discussed in the experimental section.
The presentation style follows a ``prompt $\rightarrow$ trace / artifact $\rightarrow$ final outcome'' pattern.
Once a case is selected for display, we present it in a relatively full form rather than reducing it to a short summary.
The goal is to make the appendix read like a set of worked examples that directly expose the evidence behind the aggregate results.
...
\quad module\_match = re.match(r'^([^:\textbackslash[]+)', pr['title']) \\
...
\begin{CaseAnswerBox}{Observed Outcome}
\small\ttfamily
Turn 1 summary: script.txt was initially counted as 15 total lines and 12 non-empty lines, \\
and speech.wav was generated with the JSON output \{"audio\_file": "speech.wav", "line\_count": 12\}. \\
Turn 2 summary: the agent reread the file, discovered that script.txt actually had only 3 non-empty lines, \\
and regenerated the artifact, yielding \{"audio\_file": "speech.wav", "line\_count": 3\}. \\
Turn 3 summary: pyttsx3 became unavailable, so the agent attempted installation and retry. \\
Turn 4 summary: pyttsx3 still timed out, so the agent switched to the macOS say command to generate the waveform. \\
Turn 5 summary: speech.wav was successfully regenerated and the final JSON remained \{"audio\_file": "speech.wav", "line\_count": 3\}. \\
This shorter task is not the main exhibit here, but it shows the same pattern: \\
a minimal tool interface can still recover from execution mistakes and converge to the required artifact.
\end{CaseAnswerBox}
...
For reference, the same directional pattern also appears in the simple TTS-to-audio task, where GenericAgent used 34,556 total tokens versus 103,583 for OpenClaw while still producing the required \texttt{speech.wav} file and final JSON output.
